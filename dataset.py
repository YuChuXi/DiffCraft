...
class BlockStateConverter:
    ...
    def get_block_idx(self, block: BlockState) -> int:
        return self.block_table[block.id] # 如果这里报错，说明该更新表了

    def get_block_state_idx(self, block: BlockState) -> int:
        return set([self.state_table[s] for s in self.get_block_state(block)]) # 如果这里报错，说明该更新表了
    ...
    def __init__(self, cache_pth: bool = True, all_in_mem: bool = True):
        ...
        self.sample_name_list = list(
            dict.fromkeys(
                ...
                + [i[: -len(PTH)] for i in os.listdir("dataset") if i.endswith(PTH)]
            )
        )
        ...
        for s in tqdm(self.sample_name_list, desc="Loading litematics"):
            if os.path.exists(f"dataset/{s}{PTH}"):
                try:
                    if all_in_mem:
                        self.sample_map_list[s] = torch.load(f"dataset/{s}{PTH}")
                    continue
            ...
            if cache_pth:
                torch.save(map.cpu(), f"dataset/{s}{PTH}")
